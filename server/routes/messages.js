...
router.post("/channels/:id", authenticateToken, async (req, res) => {
  try {
    ...
    console.log(
      ...
      message.schema.options.timestamps
    );
...
router.put("/:id/flag", authenticateToken, async (req, res) => {
  try {
    ...
    if (alreadyFlagged) {
      return res
        .status(400)
        .json({ message: "Message already flagged by you" });
    }
...
router.delete("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    console.log(
      "Emitted message-deleted event to channel:",
      message.channel_id
    );
    ...
    console.log(
      `Channel room ${channelRoom} has ${room ? room.size : 0} users`
    );
