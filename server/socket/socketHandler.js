...
const joinUserChannels = async (socket) => {
  try {
    ...
    console.log(
      `User ${socket.user.username} joining channels:`,
      channels.map((c) => c._id)
    );
    channels.forEach((channel) => {
      socket.join(`channel-${channel._id}`);
      console.log(
        `User ${socket.user.username} joined channel room: channel-${channel._id}`
      );
    });
  } catch (error) {
...
const handleTyping = (io, socket, data, isTyping) => {
  ...
  const channelTypingUsers = Array.from(typingUsers.values())
    .filter(
      (user) => user.channelId === channelId && user.userId !== socket.userId
    )
...
const handleJoinChannel = async (socket, channelId) => {
  try {
    console.log(
      `User ${socket.user.username} attempting to join channel: ${channelId}`
    );
    const channel = await Channel.findById(channelId);
    if (channel && channel.isMember(socket.userId)) {
      socket.join(`channel-${channelId}`);
      console.log(
        `User ${socket.user.username} joined channel room: channel-${channelId}`
      );
      socket.emit("channel-joined", { channelId });
    } else {
      console.log(
        `User ${socket.user.username} cannot join channel ${channelId}:`,
        {
